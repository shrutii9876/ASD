...
void File::insert() {
    ...
    int position =(fout.tellp())/(sizeof(r));
    fout.close();
    fout.open("IndexFile", ios::ate | ios::app);
    fout.write((char *)&position, sizeof(int));
    fout.close();
}
...
int File::Delete(int id) {
    Record r;
    fs.open("IndexFile", ios::ate | ios::app);
    int position;
    int flag = 0;
    fin.open("EmployeeDB");
    ...
    fin.seekg(0, ios::beg);
    while (fin.read((char *)&r, sizeof(r))) {
    	position=fin.tellg()/sizeof(r);
        if (r.getIdno() == id) {
            flag = 1;
            fs.seekp((position - 1)*sizeof(int));
            fs.write((char*)&position,sizeof(int));
